...
import { salesInvoices, customers, stockItems, formatCurrency } from '@/data/mockData';
...
export function SalesModule() {
  ...
  if (activeSubView === 'create') return <SalesVoucher />;
  return <SalesList />;
}
...
function SalesList() {
  ...
  const filtered = salesInvoices.filter((s) => {
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto">
      <PageHeader
        title="Sales"
        description={`${salesInvoices.length} invoices · Total: ${formatCurrency(salesInvoices.reduce((a, b) => a + b.grandTotal, 0))}`}
        action={{ label: 'New Invoice', onClick: () => setActiveSubView('create'), shortcut: 'F8' }}
      />

      <div className="flex flex-wrap items-center gap-3 mb-4">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
          <Input placeholder="Search invoices..." className="h-8 pl-8 text-sm" value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} />
        </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="text-xs h-9 cursor-pointer" onClick={() => handleSort('invoiceNumber')}>
                <span className="flex items-center gap-1">Invoice <ArrowUpDown size={12} /></span>
              </TableHead>
          ...
          <TableBody>
            {paginated.length === 0 ? (
              <TableRow>
                <TableCell colSpan={9}>
                  <EmptyState icon={<ShoppingCart size={24} />} title="No sales found" description="Create your first sales invoice." action={{ label: 'New Invoice', onClick: () => setActiveSubView('create') }} />
                </TableCell>
              </TableRow>
            ) : (
              paginated.map((inv) => (
                <TableRow key={inv.id} className="hover:bg-accent/50">
                  <TableCell className="text-xs py-2.5 font-medium text-primary">{inv.invoiceNumber}</TableCell>
                  <TableCell className="text-xs py-2.5 font-mono">{inv.date}</TableCell>
                  <TableCell className="text-xs py-2.5 font-medium">{inv.customerName}</TableCell>
                  <TableCell className="text-xs py-2.5"><StatusBadge status={inv.paymentType} /></TableCell>
                  <TableCell className="text-xs py-2.5 text-right tabular-nums font-mono">{formatCurrency(inv.subtotal)}</TableCell>
                  <TableCell className="text-xs py-2.5 text-right tabular-nums font-mono text-muted-foreground">{formatCurrency(inv.cgst + inv.sgst + inv.igst)}</TableCell>
                  <TableCell className="text-xs py-2.5 text-right tabular-nums font-mono font-semibold">{formatCurrency(inv.grandTotal)}</TableCell>
                  <TableCell className="text-xs py-2.5"><StatusBadge status={inv.status} /></TableCell>
                  <TableCell className="text-xs py-2.5 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <Button variant="ghost" size="icon" className="h-6 w-6"><Download size={13} /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))
...
function SalesVoucher() {
  ...
  const addItem = () => {
    setLineItems([...lineItems, { itemId: '', quantity: 1, rate: 0, discount: 0, gstRate: 5, amount: 0 }]);
  };
  ...
  const updateItem = (index: number, field: string, value: string | number) => {
    ...
    if (field === 'itemId') {
      ...
      if (item) {
        updated[index].rate = item.sellingPrice;
        updated[index].gstRate = item.gstRate;
      }
    }
  ...
  const handleSave = () => {
    ...
    if (lineItems.every((i) => !i.itemId)) { toast.error('Please add at least one item'); return; }
  ...
  return (
    <div className="p-6 max-w-[1200px] mx-auto">
      <PageHeader title="Create Sales Invoice" showBack description="INV-2025-009" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="min-w-0">
                  ...
                  <Select value={selectedCustomer} onValueChange={setSelectedCustomer}>
                    <SelectTrigger className="h-9 mt-1 w-full"><SelectValue placeholder="Select customer" /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="min-w-0">
                  ...
                  <Input type="date" className="h-9 mt-1 w-full" value={invoiceDate} readOnly />
                </div>
                <div className="min-w-0">
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-xs h-9 min-w-[100px] text-right">Rate</TableHead>
                      <TableHead className="text-xs h-9 min-w-[80px] text-right">Disc.</TableHead>
                  ...
                  <TableBody>
                    {lineItems.map((item, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell className="py-1.5">
                          <Input type="number" className="h-8 text-xs text-right tabular-nums" value={item.discount} onChange={(e) => updateItem(index, 'discount', e.target.value)} />
                        </TableCell>
          ...
          <Card className="sticky top-6">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="tabular-nums font-mono">{formatCurrency(Math.round(totalSgst))}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">IGST</span>
                <span className="tabular-nums font-mono">{formatCurrency(0)}</span>
              </div>
              ...
              <div className="space-y-2 pt-2">
                ...
                <Button variant="outline" className="w-full h-9"><Download size={14} className="mr-2" />Download PDF</Button>
              </div>
            </CardContent>
          </Card>
        </div>
